...
db.serialize(() => {
  ...
  const cols = [
      ...
      {table: 'tests', col: 'question_count', type: 'INTEGER DEFAULT 0'},
...
app.get('/api/tests', (req, res) => {
  db.all("SELECT id, title, creator_id, question_count, questions FROM tests ORDER BY id DESC", [], (err, rows) => {
      ...
      const results = rows.map(r => {
          ...
          if(!count) { try { count = JSON.parse(r.questions).length; } catch(e) { count = 0; } }
...
app.post('/api/tests', (req, res) => {
  ...
  db.run("INSERT INTO tests (title, questions, creator_id, question_count) VALUES (?, ?, ?, ?)", 
    ...
    function(err) {
      if (err) { console.error("SAVE ERROR:", err.message); return res.status(500).json({ error: err.message }); }
...
// === NEW: STUDENT SPECIFIC SUBMISSIONS ===
app.get('/api/student-submissions/:studentId', (req, res) => {
  const sql = `
    SELECT s.id as submissionId, s.submitted_at as submittedAt, s.auto_grade as autoGrade, 
           s.manual_grade as manualGrade, s.teacher_comment as teacherComment, t.title as testTitle
    FROM submissions s
